...
**运行时常量池(Runtime Constant Pool)是方法区的一部分**。**Class文件中**除了有类的版本/字段/方法/接口等描述信息外，**还有一项信息是常量池表**(Constant Pool Table)，**用于存放编译期生成的各种字面量和符号引用这部分内容将在类加载后进入方法区的运行时常量池中存放**，**JVM为每个已加载的类型(类或接口）都维护一个运行时常量池**，在加载类和接口到虚拟机后创建。所以运行时常量池相对于Class文件常量池的另一重要特性:具备动态性
